...
file1= csv.reader(open('Wine_Quality_Red.csv'), delimiter=',', quotechar='"')
...
if norm_features==1:
    ...
    X = data_np
    data_np = sklearn.preprocessing.scale(X, axis=0, with_mean=True, with_std=True, copy=True)
...
if feat_select==1:
    ...
    if fs_type==2:
        rgr = RandomForestRegressor(n_estimators=100, max_depth=None, min_samples_split=3,criterion='mse', random_state=100)
...
if binning==0 and cross_val==1:
    ...
    start_ts=time.time()
    clf = RandomForestRegressor(n_estimators=100, criterion='mse', max_depth=None, min_samples_split=3, max_features=0.33, random_state=rand_st)
